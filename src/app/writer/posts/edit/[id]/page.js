    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          ...
          setError('Access denied - You can only edit your own posts');
        } else {
      ...
      if (session.user.role === 'writer' && data.post.authorId._id !== session.user.id) {
        setError('Access denied - You can only edit your own posts');
        return;
      }
            ...
            >
              ...
              <p className="text-[#CFCFCF] mb-6">
                {error.includes('Access denied') 
                  ? 'You can only edit blog posts that you have authored.'
